...
theta2 <- seq(0.00002, 0.00015, 0.00002)
...
allpower_high_all <- list()
for(jj in 1:length(shape2)){
  
  allpower_high <- foreach(i = 1:myiter, .inorder = FALSE)%dopar%{
  ...
  allpower_high_all[[jj]] <- allpower_high
  
}



save(allpower_high_all, file = "simu_real_power_high_all_theta.Rdata")
